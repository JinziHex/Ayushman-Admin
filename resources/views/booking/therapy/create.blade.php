...
$(document).ready(function() {
            ...
            success: function(data) {
                ...
                if (data.error) {
                    ...
                    $.each(data.timeslots, function(key, value) {
                        ...
                        if (!selectedTimeslots[therapyID] || selectedTimeslots[therapyID].indexOf(optionText) === -1) {
                    ...
                    roomsSelect.empty().append('<option value="">--Select Room--</option>');
                     var addedRooms = []; // Array to keep track of added room names
                    
                    $.each(data.rooms, function(key, value) {
                        var optionText = value.room_name;
                    
                        // Check if the room name has already been added
                        if (addedRooms.indexOf(optionText) === -1) {
                            // Add the room name to the addedRooms array
                            addedRooms.push(optionText);
 ...
 // Track selected timeslots and update dropdown options
    $(document).on('change', '.timeslots', function() {
        var therapyID = $(this).closest('.product-row').find('.therapy_id').val();
        console.log(therapyID);
        if (!selectedTimeslots[therapyID]) {
            selectedTimeslots[therapyID] = [];
        }
        var selectedTimeslot = $(this).val();
        console.log(selectedTimeslot);
        selectedTimeslots[therapyID].push(selectedTimeslot);

        // Hide selected timeslot in all preceding rows
        var currentRow = $(this).closest('.product-row');
        currentRow.prevAll('.product-row').each(function() {
            var prevTherapyID = $(this).find('.therapy_id').val();
            if (prevTherapyID === therapyID) {
                $(this).find('.timeslots option[value="' + selectedTimeslot + '"]').remove();
            }
        });
    });
