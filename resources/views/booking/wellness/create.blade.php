                                    ...
                                    <!-- Your form or content for adding family members goes here -->
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label class="form-label">Date Of Birth</label>
                                                    <input type="date" class="form-control" name="family_member_dob" value="{{ old ('family_member_dob') }}" id="family_member_dob" placeholder="Date Of Birth">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                    ...
                    <form action="{{ route('patientbooking.wellness.booking') }}" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()" id="form-wellness">
                        ...
                        <div class="row">
                            ...
                            @if(session()->has('pharmacy_id') && session()->has('pharmacy_name') && session('pharmacy_id') != "all")
                                @php
                                    $selectedPharmacyId = session()->get('pharmacy_id');
                                    $selectedBranchId = null;
                                    if ($selectedPharmacyId && $selectedPharmacyId !== "all") {
                                        $selectedPharmacy = Mst_Pharmacy::find($selectedPharmacyId);
                                        if ($selectedPharmacy) {
                                            $selectedBranchId = $selectedPharmacy->branch;
                                            $selectedBranchName = $selectedPharmacy->branchmain['branch_name'] ?? null;
                                        }
                                    }
                                @endphp
                                <select class="form-control" required name="branch_id" id="branch_id" readonly="">
                                    @if ($selectedBranchId && $selectedBranchName)
                                        <option value="{{ $selectedBranchId }}" selected>
                                            {{ $selectedBranchName }}
                                        </option>
                                    @else
                                        <option value="">-No Branches for the selected pharmacy--</option>
                                    @endif
                                </select>
                            @else
                            ...
                            @endif
                        @elseif(in_array(Auth::user()->user_type_id, [18, 20, 21]))
                            @php
                                $selectedBranchName = optional(Auth::user()->staff->branch)->branch_name ?? null;
                                $selectedBranchID = Auth::user()->staff->branch_id ?? null;
                            @endphp
                            <select class="form-control" required name="branch_id" id="branch_id" readonly="">
                                @if ($selectedBranchName)
                                    <option value="{{$selectedBranchID}}" selected>{{ $selectedBranchName }}</option>
                                @else
                                    <option value="">-No Branches for the selected staff--</option>
                                @endif
                            </select>
                        @endif
                                </div>
                                                ...
                                                <tr id="productRowTemplate" class="product-row">
                                                    ...
                                                    <td>
                                                        <input type="text" id="booking_fee" class="form-control booking_fee" required name="booking_fee[]" placeholder="Booking Fee" value="" readonly>
                                                         ...
                                                         <td><select class="form-control rooms" required name="rooms[]" id="rooms">
                                                            <option value="">--Select Room--</option>
                                                        </select></td>
                                                    <td><select class="form-control timeslots" required name="timeslots[]" id="timeslots">
                                                            <option value="">--Select Timeslot--</option>
                                                        </select></td>
                                                        <td><select class="form-control staffs" required name="staff_id[]" id="staff_id">
                            ...
                            <div class="col-md-12">
                                <div class="form-group" style="border: 1px solid #000;">
                                    <span style="margin-left: 10px;
                                        ...
                                        Details : Package name: <span id="package_name"></span> | Start Date: <span id="start_date"></span> | Expiry: <span id="expiry_date"></span>
                                        <br>
                                        <ul id="wellness_list" style="list-style-type: none; padding-left: 0;">
                                            <!-- Wellness details will be appended here dynamically -->
                                        </ul>
                                        </span>
                                        <input type="hidden" class="is_membership_available" name="is_membership_available">
                                </div>
                            </div>
                        </div>
                        <input type="hidden" class="custom-control-input membershipIds" name="membershipId"  id="membershipId">
                        <input type="hidden" class="custom-control-input countMembershipIds" name="countMembershipIds"  id="countMembershipIds">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <center>
                                        ...
                                        <a class="btn btn-danger" href="{{route('bookings.wellness.index')}}"> <i class="fa fa-times"></i>
                                            Cancel</a>
                                    </center>
                                </div>
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('change', '.wellness_id', function() {
            ...
            if(!patientId){
            swal("Error", "Patient  is required", "error");
            $(this).val(null);
            }
            ...
            if (wellnessID && bookingDate) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var roomsSelect = row.find('.rooms');
                        ...
                        if (data.error) {
                            ...
                            $.each(data.timeslots, function(key, value) {
                                var optionText = value.time_from + ' - ' + value.time_to;
                                $('#timeslots').append($('<option>', {
                                    value: value.timeslot,
                                    text: optionText
                                }));
                            });
                            roomsSelect.empty().append('<option value="">--Select Room--</option>');
                            $.each(data.timeslots, function(key, value) {
                                 var optionText = value.therapy_room_name;
                                $('#rooms').append($('<option>', {
            ...
            if (wellnessId !== '' && membershipId !== '') {
                $.ajax({
                    url: '/get-available-balance',
                    type: 'GET',
                    data: {wellness_id: wellnessId,patient_id:patientId,membership_id:membershipId},
                    success: function (response) {
                        console.log('Available Balance:', response);
                        
                        $('.countMembershipIds').val(response.balance);
                       setTimeout(function() {
                           calculatePayableAmount(0, well)
                       }, 1000);
                    },
        ...
        $(document).on('change', '#patient_id', function() {
            ...
            if (patientId) {
                ...
                $.ajax({
                    url: '{{ route('wellness.getMembershipWellness') }}',
                    type: "GET",
                    data: {
                    ...
                    success: function(data) {
                         if ('membership' in data) {
                            if (data.membership.package_title !== null &&
                                data.membership.start_date !== null &&
                                data.membership.expiry_date !== null) {
                                $(".is_membership_available").val(0)
                                ...
                                $('#membershipId').val(data.membership.membership_package_id);
                    
                                // Clear existing wellness list
                                $('#wellness_list').empty();
                    
                                // Append wellness details to the list
                                $.each(data.wellness_details, function(index, wellness) {
                                    var listItem = $('<li>').text('Wellness name: ' + wellness.wellness_name + ' | Max Limit: ' + wellness.max_limit + ' | Available Balance: ' + wellness.available_balance);
                                    $('#wellness_list').append(listItem);
                                });
                            } else {
                        ...
                        } else if ('message' in data) { // Check if a message is returned
                            // Display the message
                            $('#membership_details').hide();
                            $(".is_membership_available").val(0)
                            calculatePayableAmount(0)
                ...
                $('#wellness_list').hide();
             $('#membership_details').hide();
             $("family_member select").empty();
             $(".wellness_id").val('');
              $('.wellness_id').trigger('change');
            }
        ...
        function calculatePayableAmount(e, item) {
           // alert(e)
            var totalBookingFee = 0;
            ...
            var balance = $("#countMembershipIds").val();
            //alert(balance)
           ...
           if(balance) {
            if(balance <= 0) {
                ...
                //swal("Error", "The selected wellness option is either not included in your membership package or not available.", "error");
                swal({
                  title: 'The selected wellness option is either not included in your membership package or not available',
                ...
                function confirmFunction() {
                    console.log('cnfirm button clicked');
                  $('#payable_amount').val(totalBookingFee.toFixed(2));
