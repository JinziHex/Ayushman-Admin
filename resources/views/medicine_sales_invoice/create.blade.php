...
table#productTable tbody tr:nth-child(1) button {
    pointer-events: none;
}
                              ...
                              <tbody>
                                 <tr id="productRowTemplate" >
                                    <td  style="min-width: 300px;">
                                       <select class="form-control medicine-name medsearch medicine-select" name="medicine_id[]">
                                          <option value="" selected disabled>Please select medicine</option>
                                          @foreach($medicines as $medicine)
                                          <option value="{{ $medicine->id }}">{{ $medicine->medicine_name}} - {{ $medicine->medicine_code}} </option> @endforeach
                                       </select>
                                    </td>
                                    <td class="medicine-batch-no"><input type="text" class="form-control" value="" name="batch_no[]" readonly></td>
                                    <td class="medicine-quantity"><input type="number"  class="form-control" value="" name="quantity[]" oninput="calculateAmount(this)" min="1"></td>
                           ...
                           <div class="card-body">
                              <table style="width: 100%;">
                                 ...
                                 <tr>
                                    ...
                                    <td style="text-align: right;"><strong class="round-amount">0</strong><input type="hidden" id="round-amount-input" name="round_amount" value="0"></td>
                                 </tr>
                                 <tr>
                                    <td><strong>Discount Percentage</strong></td>
                                    <label class="form-label">Maximum Discount: {{ $discount_percentage}}%</label>
                                    <td style="text-align: right;"><strong class="discount-amount"></strong><input type="text" class="form-control" id="discount-amount-input" name="discount_amount" placeholder="0" readonly 
...
<script>
   ...
   $(document).ready(function() {
      ...
      function updateTotalAmount() {
         var subTotal = parseFloat($('.tot').val()) || 0;
         var taxAmount = parseFloat($('.tax-amount').val()) || 0;
    ...
    $.ajax({
      ...
      function updateBalAmount(elem=null) {
         ...
         //upadatedBal=upadatedBal.toFixed(2)
         $(".balance-payable b").text(" " + upadatedBal.toFixed(2))
      }
      ...
      $("#productTable tbody tr").each(function() {
         ...
         if (Math.abs(calculatedAmount - amount) > epsilon) {
            ...
            if(totalRows > 1) {
                ...
                var select = parseFloat($(this).find('select[name="medicine_id[]"]').val());  
...
//alert("t")
      ...
      $.ajax({
         ...
         success: function(response) {
            ...
            response.data.forEach(function(item, index) {
               //alert(item.id)
               var disabledAttribute = item.medicine_current_stock === 0 ? 'disabled' : '';
               var row = `<tr>
                    ...
                    <td class="batch-medicine-unit-price">${item.medicine_unit_price != null ? item.medicine_unit_price : '0.00'}</td>
                    <td class="batch-medicine-unit-price2">${item.medicine_with_tax != null ? item.medicine_with_tax : '0.00'}</td>
                    <td class="batch-medicine-tax-rate">${item.medicine_tax_rate}</td>
                    <td class="radio-batch-btn"><input type="radio" value="${item.id}" name="selected_batch" ${disabledAttribute}></td>
                </tr>`;
      ...
      $(document).on('change', '.radio-batch-btn', function() {
         ...
         $(".selectedCls").find(".medicine-quantity input").attr("min", 1);
   ...
   $(document).on('click', '#close-modal', function() {
      ...
      if (selectedValue.length != 0) {
         ...
         if (j > 1) {
            ...
            selected.each(function(index) {
              ...
              if (index == 0) {
                ...
                var amt =  parseFloat(ro.find('input[name="rate[]"]').val());
                var currentQty = parseFloat(ro.find('input[name="quantity[]"]').val());
                var x = currentQty+1
                
                var y = $('tr').find('input[name="quantity[]"]').attr('max');
                if(x <= y) {
                ro.find('input[name="quantity[]"]').val(x);
                 ro.find(".medicine-amount input").val(x * amt)
                } else {
                    var alertDiv = $('<div>').text('Value cannot be greater than ' + y).addClass('alert-div');
                     ro.find(".medicine-quantity").append(alertDiv);
        ...
        //  var x = sum + totalTax
        //  x = x.toFixed(2)
         $(".total-amount").text((sum + totalTax).toFixed(2));
         $('#total-amount-input').val((sum + totalTax).toFixed(2));
       ...
       //$(".round-amount").text(payable - Math.floor(payable));
        
          var xyz = payable - Math.floor(payable)
         $(".round-amount").text(xyz.toFixed(2));
        ...
        payable = payable.toFixed(2); 
         ...
         $(".payable-amount b").text('' + payable)
         $(".balance-payable b").text('0.00')
         $("#pay1").val(payable);
         
         $(".paid-amount:first").val(payable);
         
         // $(".paid-amount").val(payable)
         $("#discount-amount-input").removeAttr("readonly");
      }
   ...
   function calculateAmount(input) {
      ...
      if (!isNaN(quantity) && !isNaN(rate)) {
         ...
         amount.each(function() {
            sum1 = parseFloat($(this).val()) || 0;
            var x = parseFloat($(this).parent("td").siblings(".medicine-tax-rate").find('input').val()) || 0;
            // alert(x)
            //x = parseFloat(x) || 0;
            var tax = (sum1 * x) / 100;
            tax = tax.toFixed(2);
         ...
         x= parseFloat(sum )+ parseFloat(totalTax );
         
         
         var totalAmount = x.toFixed(2);
         
        // console.log(totalAmount)
         //console.log("tessssssssssssst"+ totalAmount)
         $(".total-amount").text(totalAmount);
         $('#total-amount-input').val(totalAmount);

         // Discount
         var totalA = parseFloat($(".total-amount").text())
         var discount =parseFloat($("#discount-amount-input").val())|| 0
         var discountT = (totalA * discount) / 100
         console.log(discount, "discount");

         var payable = totalA - discountT
         //console.log("ttttttttttttttt"+payable);
   ...
   $(document).on('click', '.no-selected-item', function() {
      ...
      newRow.removeAttr("style");
       newRow.find('.medicine-select').val('');
...
<script>
   $(document).ready(function() {
      ...
      $('#discount-amount-input, #total-amount-input').on('input', function() {
        ...
        // var xyz = payableAmount - Math.floor(payableAmount)
        //  $(".round-amount").text(xyz.toFixed(2));
        if (payableAmount - Math.floor(payableAmount) >= 0.5) {
            payableAmount = Math.ceil(payableAmount);
        } else {
            payableAmount= Math.floor(payableAmount);
        }
        payableAmount = payableAmount.toFixed(2); 
         $(".payable-amount b").text('' + payableAmount);
         $("#pay1").val(payableAmount);
         $(".paid-amount:first").val(payableAmount);
         $(".balance-payable b").text('0.00')
...
<script>
   $(document).ready(function() {
      ...
      $('#discount-amount-input, #total-amount-input, #percentage-input').on('input', function() {
         ...
         //$(".round-amount").text(paidAmount - Math.floor(paidAmount));
        //  var xyz = paidAmount - Math.floor(paidAmount)
        //  $(".round-amount").text(xyz.toFixed(2));
         
        if (paidAmount - Math.floor(paidAmount) >= 0.5) {
        ...
        paidAmount = paidAmount.toFixed(2); 
         ...
         $(".payable-amount b").text(paidAmount);

         $("#pay1").val(paidAmount);
         // $(".paid-amount").val(paidAmount.toFixed(2));
         $(".paid-amount:first").val(paidAmount.toFixed(2));
      });
...
$(document).ready(function () {
     ...
     $("#addProductBtn").click(function(event) {
         ...
         newRow.find('input[type="text"]').val('');
         newRow.find('input[type="number"]').val('');
         newRow.find('.medicine-quantity input').prop("readonly", false);
         newRow.find('input').removeAttr("disabled")
         $('.medicine-quantity input').prop('readonly', false);
         // newRow.removeAttr('style')
         newRow.find('input span').remove()
