                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                               ...
                               @if(session()->has('pharmacy_id') && session()->has('pharmacy_name') && session('pharmacy_id') != "all")
                                <select class="form-control defaultSet" name="pharmacy_id" id="pharmacy_id" required readonly>
...
<script>
    ...
    $(document).ready(function() {
        var x = $('.defaultSet').val();
        if(x) {
            wellnessList(x)
        }
        
        $('select[name="pharmacy_id"]').change(function() {
            //alert("test")
            var pharmacyId = $(this).val();
            wellnessList(pharmacyId)
        });
        function wellnessList(pharmacyId) {
            if (pharmacyId) {
                $.ajax({
                    type: 'GET',
                    url: '/get-wellness-list/' + pharmacyId,
                    success: function(data) {
                        $('#wellness_id').html('<option value="">Select Wellness</option>');
