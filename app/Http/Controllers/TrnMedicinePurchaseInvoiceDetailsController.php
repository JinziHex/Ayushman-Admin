...
class TrnMedicinePurchaseInvoiceDetailsController extends Controller
{
        ...
        try {
            ...
            ->whereIn('trn_medicine_purchase_invoices.pharmacy_id', $mappedpharma)
                ...
                if (Auth::user()->user_type_id == 18 || Auth::user()->user_type_id == 21 || Auth::user()->user_type_id == 20) {
                    ...
                    if ($branch_id) {
                        $query->where('mst_pharmacies.branch', $branch_id);
                    }
                } else {
    ...
    { 
        ...
        if ($validator->fails()) {
            ...
            $purchaseMrp = $request->input('purchase_mrp');
            $rateIncluding = $request->input('rateIncluding');
            ...
            foreach ($productIds as $key => $productId) {
                ...
                $detail->medicine_code = $medicineCodes[$key]?? null;
                ...
                $detail->purchase_mrp = $purchaseMrp[$key];
                ...
                $stock->sale_rate_with_tax = $rateIncluding[$key];
    ...
    public function getMedicineDetails($productId)
    {
        $medicineDetails = Mst_Medicine::where('mst_medicines.id', $productId)
            ->leftJoin('mst__tax__groups', 'mst_medicines.tax_id', '=', 'mst__tax__groups.id')
        ...
        if ($medicineDetails !== null) {
            return response()->json([
                ...
                'tax_rate' =>isset($totalTaxRate) ? $totalTaxRate : 0,
            ]);
        } else {
            return response()->json([
    ...
    public function destroy($id)
    {
        DB::beginTransaction();
        try {
            $invoiceList = Trn_Medicine_Purchase_Invoice_Detail::where('invoice_id', $id)->get();
    
            foreach ($invoiceList as $invoice) {
                $stock = Trn_Medicine_Stock::where('medicine_id', $invoice->product_id)
                    ->where('batch_no', $invoice->batch_no)
                    ->where('mfd', $invoice->mfd)
                    ->where('expd', $invoice->expd)
                    ->update(['current_stock' => DB::raw('current_stock - '.$invoice->quantity)]);
            }
    ...
    public function view($id)
    {
        ...
        $creditPeriod = Mst_Supplier::where('supplier_id',$purchase_invoice->supplier_id)->value('credit_period');
         $creditLimit = $purchase_invoice->credit_limit;
        $dueDate = now()->addDays($creditPeriod)->format('Y-m-d');
        if (!$creditPeriod) {
            $dueDate = now()->format('Y-m-d');
        }
        $total_paid=Trn_Medicine_Purchase_Invoice::where(['supplier_id'=>$purchase_invoice->supplier_id,'is_paid'=>0])->whereNotNULL('paid_amount')->whereNotNULL('total_amount')->sum('paid_amount');
        $total_invoiced_amount=Trn_Medicine_Purchase_Invoice::where(['supplier_id'=>$purchase_invoice->supplier_id,'is_paid'=>0])->whereNotNULL('paid_amount')->whereNotNULL('total_amount')->sum('total_amount');
        $utilizedAmount=$total_invoiced_amount-$total_paid;
        $remaining_credit_amount=$creditLimit-$utilizedAmount;
        if($remaining_credit_amount<0)
        {
        ...
        return view('medicine_purchase_invoice.view', compact('pageTitle','purchase_invoice','creditLimit','remaining_credit_amount'));
