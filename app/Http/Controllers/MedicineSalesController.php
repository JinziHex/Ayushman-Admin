...
use App\Models\mst__tax__groups;
...
class MedicineSalesController extends Controller
{
    public function index(Request $request)
    {
        ...
        $medicineSalesInvoice = $query->latest('trn__medicine__sales__invoices.updated_at')->get();
        ...
        try {
            ...
            $medicine_batch_details = Mst_Medicine::join('trn_medicine_stocks', 'mst_medicines.id', '=', 'trn_medicine_stocks.medicine_id')
                ...
                ->select(
                    ...
                    'trn_medicine_stocks.sale_rate_with_tax',
                    'mst_units.unit_name',
          ...
          if ($medicineDetails) {
            ...
            foreach ($medicine_batch_details as $medicine_batch) {
                $data[] = [
                    ...
                    'medicine_with_tax' => $medicine_batch->sale_rate_with_tax,
    ...
    public function store(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $lastInsertedId = Trn_Medicine_Sales_Invoice::insertGetId([
                ...
                'paid_amount' =>array_sum($paidAmounts),
            ...
            $unitId = $request->unit_id;
             ...
             $unit_ids = Mst_Unit::whereIn('unit_name', $unitId)->pluck('id', 'unit_name');
            foreach ($medicines as $key => $medicine) {
                $unit_id = Mst_Medicine::where('id', $medicine)->value('unit_id');
                
                   $invdetails =  Trn_Medicine_Sales_Invoice_Details::create([
                        ...
                        'rate' => $rates[$key],
                        'amount' => $amounts[$key],
                        'expiry_date' => $expds[$key],
                        'manufactured_date' => $mfds[$key],
                        'med_quantity_tax_amount' => $single_tax_amounts[$key],

                    ]);



                    $stock = Trn_Medicine_Stock::where('medicine_id', $medicine)
                        ->where('mfd', $mfds[$key])
