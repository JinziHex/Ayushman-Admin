...
class SalaryProcessingController extends Controller
{
    ...
    {
        $query = Trn_staff_salary_processing::with('staff', 'branch')->orderBy('created_at', 'DESC');
        if (Auth::user()->user_type_id == 18 || Auth::user()->user_type_id == 21 || Auth::user()->user_type_id == 20) {
            $branch_id = Auth::user()->staff->branch_id;
            if ($branch_id) {
                $query->whereHas('staff', function ($q) use ($branch_id) {
    ...
    public function getStaffLeaves($staff_id, $month)
    {
        ...
        $totalCount = Trn_Booking_Wellness_Detail::where('staff_id', $staff_id)->whereMonth('created_at', $month)->count('booking_id');
        $sumTotals = Trn_Booking_Wellness_Detail::where('staff_id', $staff_id)->whereMonth('created_at', $month)->sum('wellness_fee');
        $staff_commision = Mst_Staff::where('staff_id', $staff_id)->value('staff_commission');
        $staff_commision_type = Mst_Staff::where('staff_id', $staff_id)->value('staff_commission_type');
        if($staff_commision_type == "fixed")
        {
          $sumTotal =   $staff_commision * $totalCount;
        }
        else
        {
            $sumTotal = $sumTotals * $staff_commision /100;
        }
    ...
    {
        $query = Trn_Staff_Advance_Salary::orderBy('created_at', 'DESC');
        if (Auth::user()->user_type_id == 18 || Auth::user()->user_type_id == 21 || Auth::user()->user_type_id == 20) {
            $branch_id = Auth::user()->staff->branch_id;
            if ($branch_id) {
                $query->whereHas('staff', function ($q) use ($branch_id) {
