...
class BookingController extends Controller
{
    ...
    {
        $bookingsQuery = Trn_Consultation_Booking::select('trn_consultation_bookings.*', 'mst_patients.patient_name', 'mst_patients.patient_code', 'mst_staffs.staff_name', 'mst_branches.branch_name', 'booking_status.master_value')
        ...
        if (Auth::user()->user_type_id == 18 || Auth::user()->user_type_id == 21 || Auth::user()->user_type_id == 20) {
            ...
            if ($branch_id) {
                $bookingsQuery->where('mst_branches.branch_id', $branch_id);
            }
        } else {
        ...
        $bookingsQuery->when($request->filled('appointment_from_date'), function ($query) use ($request) {
            $query->where('booking_date', '>=', $request->input('appointment_from_date'));
        })
        ->when($request->filled('appointment_to_date'), function ($query) use ($request) {
            $query->where('booking_date', '<=', $request->input('appointment_to_date'));
        })->when(!$request->filled('appointment_from_date') && !$request->filled('appointment_to_date'), function ($query) {
    ...
    public function getBookingFee(Request $request)
    {
        ...
        $availableTimeSlots = Mst_Staff_Timeslot::where('staff_id', $staffId)
        ->whereHas('weekDay', function ($query) use ($weekday) {
            $query->where('master_id', 3)
                ->where('master_value', $weekday);
        })
        ->with(['timeSlot', 'bookings'])
        ->where(function ($query) use ($bookingDate) {
            $query->whereDoesntHave('bookings', function ($query) use ($bookingDate) {
                $query->where('booking_date', $bookingDate);
            })->orWhereHas('bookings', function ($query) use ($bookingDate) {
                $query->where('booking_date', $bookingDate)->havingRaw('MAX(token_number) < no_tokens');
            });
        })
        ->get();
        
        foreach ($availableTimeSlots as $timeslot) {
            $maxTokenNumber = $timeslot->bookings->max('token_number');
            $availableTokens = $timeslot->no_tokens - $maxTokenNumber;
            $timeslot->available_tokens = $availableTokens;
        }
        
        
       if ($availableTimeSlots->isEmpty()) {
        return response()->json(['error_message' => 'No tokens available for the selected Doctor']);
        }

        $formattedTimeSlots = $availableTimeSlots->map(function ($timeslot) {
            return [
                'slot_id' => $timeslot->id,
                'slot_name' => $timeslot->timeSlot->slot_name,
                'time_from' => $timeslot->timeSlot->time_from,
                'time_to' => $timeslot->timeSlot->time_to,
                'available_tokens' => $timeslot->available_tokens,
            ];
        });
    ...
    public function patientBooking(Request $request)
    {
        ...
        $existingBooking = Trn_Consultation_Booking::where('booking_date', $request->booking_date)
                            ->where('staff_timeslot_id', $request->timeslots)
                            ->orderBy('token_number', 'desc')
                            ->first();
        if ($existingBooking) {
        $tokenNumber = $existingBooking->token_number + 1;
        } else {
            $tokenNumber = 1;
        }
        ...
        $booking->staff_timeslot_id = $request->timeslots;
    ...
    {
        $bookingsQuery = Trn_Consultation_Booking::select('trn_consultation_bookings.*', 'mst_patients.patient_name', 'mst_branches.branch_name', 'booking_status.master_value')
                        ->join('mst_patients', 'mst_patients.id', '=', 'trn_consultation_bookings.patient_id')
                        ->join('mst_branches', 'mst_branches.branch_id', '=', 'trn_consultation_bookings.branch_id')
                        ->join('mst_master_values as booking_status', 'booking_status.id', '=', 'trn_consultation_bookings.booking_status_id')
    ...
    public function WellnessCreate(Request $request)
    {
        return view('booking.wellness.create', [
            'branches' => Mst_Branch::where('is_active', '=', 1)->orderBy('branch_name', 'ASC')->get(),
            'patients' => Mst_Patient::orderBy('patient_name', 'ASC')->get(),
            'staffs'  => Mst_Staff::where('staff_type', 19)->get(),
            'paymentType' => Mst_Master_Value::where('master_id', 25)->pluck('master_value', 'id'),
            'genders' => Mst_Master_Value::where('master_id', 17)->pluck('master_value', 'id')->toArray(),
    ...
    public function getWellnessList(Request $request)
    {
        ...
        $branchId = $request->input('branch_id');
        $wellness = Mst_Wellness::join('trn_wellness_branches', 'mst_wellness.wellness_id', '=', 'trn_wellness_branches.wellness_id')
            ->where('mst_wellness.is_active', 1)
            ->where('trn_wellness_branches.branch_id', $request->branch_id)
            ->select('mst_wellness.wellness_id', 'mst_wellness.wellness_name')
            ->get();
    ...
    public function getMembershipWellness(Request $request)
    {
       $patientId = $request->input('patient_id');
        $membership = Mst_Patient_Membership_Booking::where('patient_id', $patientId)
            ->orderBy('created_at', 'desc')
            ...
            $start_date = isset($membership->start_date) ? Carbon::parse($membership->start_date)->toDateString() : null;
            $expiry_date = isset($membership->membership_expiry_date) ? Carbon::parse($membership->membership_expiry_date)->toDateString() : null;
            $membershipUser = isset($membership->membership_package_id) ? $membership->membership_package_id : null;
            $membershipInfo = [
                'package_title' => $membership->membershipPackage->package_title ?? null,
                'start_date' => $start_date,
                'expiry_date' => $expiry_date,
                'membership_package_id' => $membershipUser,
                
            ];
             // B. Get "Included Wellness Info" of the membership
            $membershipPackageId = $membershipUser;
            $includedWellnesses = Mst_Membership_Package_Wellness::where('package_id', $membershipPackageId)
                ->with('wellness') 
                ->get()
                ->filter(function ($item) {
                    return optional($item->wellness)->exists;
                });
                
            $wellnessDetails = [];
            foreach ($includedWellnesses as $includedWellness) {
                $wellnessId = $includedWellness->wellness_id;
                $wellnessName = $includedWellness->wellness->wellness_name;
                $maxUsageLimit = $includedWellness->maximum_usage_limit;
            
                // D. Get the available balance for each included wellness
                $availableBalance = $this->getAvailableBalance($patientId, $wellnessId,$membershipPackageId);
            
                $wellnessDetails[] = [
                    'wellness_id' => $wellnessId,
                    'wellness_name' => $wellnessName,
                    'max_limit' => $maxUsageLimit,
            ...
            return response()->json([
                ...
                'wellness_details' => $wellnessDetails,
            ]);
    }
    
   private function getAvailableBalance($patientId, $wellnessId, $membershipPackageId)
    {
        // D1. Get the selected booking IDs
        $selectedBookingIds = Trn_Consultation_Booking::where('patient_id', $patientId)
            ->where('booking_type_id', 85) // Wellness booking
            ->where('is_membership_available', 1)
            ->pluck('id');
    
        // D2. Iterate through the trn__booking__wellness__details for selected booking IDs
        $wellnessCount = Trn_Booking_Wellness_Detail::whereIn('booking_id', $selectedBookingIds)
            ->where('wellness_id', $wellnessId)
            ->count();
        $maxLimit = Mst_Membership_Package_Wellness::where('package_id', $membershipPackageId)->where('wellness_id', $wellnessId)
        ->value('maximum_usage_limit');
    
        // E. Calculate available balance
        $availableBalance = $maxLimit - $wellnessCount;
    
        return $availableBalance;
    }
    
    public function getWellnessAvailableBalance(Request $request)
    {
        
        $patientId = $request->patient_id;
        $membershipPackageId = $request->membership_id;
        $wellnessId = $request->wellness_id;
    
        $availableBalance = $this->getAvailableBalance($patientId, $wellnessId, $membershipPackageId);
    
      return response()->json(['balance' => $availableBalance]);
    }
    ...
    public function patientWellnessBooking(Request $request)
    {
        ...
        $booking->booking_status_id = 87;
        $booking->is_membership_available = 1;
        $booking->save();
        ...
        Trn_Booking_Wellness_Detail::create([
                ...
                'rooms' => $request['rooms'][$index],
                'staff_id' => $request['staff_id'][$index],
            ]);
        }
  ...
  if ($request->noBill != 1) {
    ...
    {
        $patient_array=Trn_Consultation_Booking::query()->pluck('patient_id');
        $patient_ids = $patient_array->unique()->values()->all();
        return view('booking.therapy-refund.create', [
            'patients' => Mst_Patient::orderBy('patient_name','ASC')->whereIn('id',$patient_ids)->get(),
            'paymentType' => Mst_Master_Value::where('master_id', 25)->pluck('master_value', 'id'),
    ...
    public function fetchRefundBookings(Request $request)
    {
        ...
        $bookings = Trn_Consultation_Booking::where('patient_id', $patientId)
            ...
            ->where(function($query) {
                $query->where('is_membership_available', null)
                      ->orWhere('is_membership_available', 0);
            })
            ->where('is_billable','=',1)
